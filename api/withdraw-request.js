...
// Kept for the daily-limit check below; the fixed 8AM-5PM window itself is no longer
// enforced here — see the admin-toggle check inside the handler instead.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Withdrawal availability is controlled by the admin toggle (settings/withdrawalConfig),
    // not a fixed time window — matches admin-withdrawals.html's on/off switch exactly.
    const configSnap = await db.collection('settings').doc('withdrawalConfig').get();
    const withdrawalsEnabled = configSnap.exists ? configSnap.data().enabled !== false : true;
    if (!withdrawalsEnabled) {
      return res.status(400).json({ error: 'Withdrawals are temporarily disabled by admin. Please try again later.' });
    }
