...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const transferId = flwData.data?.id || null;
      ...
      // The response above only means "Flutterwave accepted the request into their queue" —
      // it does NOT mean the money actually moved. Things like insufficient balance often get
      // caught within a few seconds, so we poll once before telling the admin anything, instead
      // of reporting a false "success" for a transfer that's actually about to fail.
      let finalStatus = 'processing';
      let finalMessage = 'Transfer queued — Flutterwave has not yet confirmed final delivery. Use "Check Status" shortly to confirm.';

      if (transferId) {
        await new Promise((resolve) => setTimeout(resolve, 3000)); // brief wait before checking
        try {
          const statusResponse = await fetch(`https://api.flutterwave.com/v3/transfers/${transferId}`, {
          ...
          if (flwTransferStatus === 'FAILED') {
            // Caught it fast — refund immediately instead of leaving it stuck as "processing".
            ...
            return res.status(502).json({ error: 'Transfer failed and the user has been refunded: ' + reason });
          }

          if (flwTransferStatus === 'SUCCESSFUL') {
            await withdrawalRef.update({ status: 'success' });
            finalStatus = 'success';
            finalMessage = 'Transfer completed successfully.';
          }
          // NEW/PENDING: leave as 'processing' — genuinely still in flight, not a failure.
        } catch (pollError) {
          console.error('Status poll error (non-fatal):', pollError);
          // If the poll itself fails, we still leave the withdrawal as 'processing' — the
          // Check Status button can be used to try again later.
        }
      }
      ...
      return res.status(200).json({ status: finalStatus, message: finalMessage });
